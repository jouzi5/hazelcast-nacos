...
public class NacosDiscoveryStrategy extends AbstractDiscoveryStrategy {
    ...
    public void start() {
        ...
        if (serverAddr == null || username == null || password == null) {
            throw new RuntimeException("nacos properties [nacos_serverAddr, nacos_username, nacos_password] must be set");
        }
        ...
        if (this.thisNode != null) {
            String instanceName = Optional.ofNullable(thisNode.getProperties().get("nacosInstanceName"))
                    .map(Object::toString).orElse(null);
            if (!StringUtil.isNullOrEmpty(instanceName)) {
                this.instanceName = instanceName;
            }
            try {
                nacosNamingService.registerInstance(this.instanceName, getOrNull(NacosDiscoveryProperties.NACOS_GROUP),
                        this.thisNode.getPrivateAddress().getHost(), thisNode.getPrivateAddress().getPort());
            } catch (NacosException e) {
